namespace MurshisoftData.Caching
{
    using System;
    ...
    /// <summary>
    /// LRU cache - evicts least recently used entries first based on access time.
    /// </summary>
    public class LRUCache<T1, T2> : CacheBase<T1, T2>
    {
        ...
        public LRUCache(int capacity, int evictCount, IEqualityComparer<T1> comparer = null)
        {
        ...
        /// <param name="comparer">Optional equality comparer for keys.</param>
        public LRUCache(int capacity, int evictCount, IPersistenceDriver<T1, T2> persistence, IEqualityComparer<T1> comparer = null)
        {
            if (capacity < 1) throw new ArgumentOutOfRangeException(nameof(capacity));
            if (evictCount < 1) throw new ArgumentOutOfRangeException(nameof(evictCount));
            if (evictCount > capacity) throw new ArgumentOutOfRangeException(nameof(evictCount));
        ...
        public override async Task AddReplaceAsync(T1 key, T2 val, DateTime? expiration = null, CancellationToken cancellationToken = default)
        {
            ...
            lock (_CacheLock)
            {
                ...
                // Capacity-based eviction (LRU: evict by LastUsed)
                ...
                // Memory-based eviction (LRU: evict by LastUsed)
                if (MaxMemoryBytes > 0)
                {
