namespace CosmoS3.Classes
{
    ...
    internal class AuthManager
    {
        ...
        internal RequestMetadata AuthorizeServiceRequest(S3Context ctx, RequestMetadata md)
        {
            ...
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Http.Request.Method.ToString() + " " + ctx.Http.Request.Path + "] AuthorizeServiceRequest ";

            #region Check-for-Admin-API-Key

            if (ctx.Http.Request.Headers.ContainsKey(_Settings.HeaderApiKey))
            {
        ...
        internal RequestMetadata AuthorizeBucketRequest(S3Context ctx, RequestMetadata md)
        {
            ...
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Http.Request.Method.ToString() + " " + ctx.Http.Request.Path + "] AuthorizeBucketRequest ";
            bool allowed = false;

            bool GroupMatches(string groupValue, string groupName)
            {
                return !String.IsNullOrEmpty(groupValue)
                    && groupValue.Contains(groupName, StringComparison.OrdinalIgnoreCase);
            }

            bool GuidMatches(string left, string right)
            {
                ...
                switch (ctx.Request.RequestType)
                {
                    ...
                    case S3RequestType.BucketReadVersions:
                        allowed = md.BucketAcls.Exists(
                            b => GroupMatches(b.UserGroup, "AllUsers")
            ...
            if (md.Bucket != null)
            {
                if (GuidMatches(md.Bucket.OwnerGUID, md.User.GUID))
                {
                    md.Authorization = AuthorizationResult.PermitBucketOwnership;
                    return md;
                }
            ...
            if (md.BucketAcls != null && md.BucketAcls.Count > 0)
            {
                switch (ctx.Request.RequestType)
                {
                    ...
                    case S3RequestType.BucketReadVersions:
                        allowed = md.BucketAcls.Exists(
                            b => GroupMatches(b.UserGroup, "AuthenticatedUsers")
                            && (b.PermitRead || b.FullControl));
                        break;
            ...
            if (md.BucketAcls != null && md.BucketAcls.Count > 0)
            {
                switch (ctx.Request.RequestType)
                {
                    ...
                    case S3RequestType.BucketReadVersions:
                        allowed = md.BucketAcls.Exists(
                            b => GuidMatches(b.UserGUID, md.User.GUID)
                            && (b.PermitRead || b.FullControl));
                        break;
        ...
        internal RequestMetadata AuthorizeObjectRequest(S3Context ctx, RequestMetadata md)
        {
            ...
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Http.Request.Method.ToString() + " " + ctx.Http.Request.Path + "] AuthorizeObjectWriteRequest ";
            bool allowed = false;
            ...
            if (md.Obj != null)
            {
                if (GuidMatches(md.Obj.OwnerGUID, md.User.GUID))
                {
                    md.Authorization = AuthorizationResult.PermitObjectOwnership;
                    return md;
                }
