...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        {
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Request.RequestType.ToString() + "] ";
        ...
        internal async Task<DeleteResult> DeleteMultiple(S3Context ctx, DeleteMultiple dm)
        {
            ...
            {
                foreach (CosmoS3.S3Objects.Object curr in dm.Objects)
                {
                    long versionId = 1;
        ...
        internal async Task Write(S3Context ctx)
        {
            ...
            if (obj == null)
            {
                ...
                {
                    obj.AuthorGUID = "0.0.0.0" + ":" + 0;
                    obj.OwnerGUID = "0.0.0.0" + ":" + 0;
                }

                obj.GUID = Guid.NewGuid().ToString();
                obj.Version = 1;
                obj.BlobFilename = obj.GUID;
                obj.ContentLength = (long)ctx.Http.Request.Body.Length;
                obj.ContentType = ctx.Http.Request.Headers.GetValueOrDefault("content-type", "");
                obj.CreatedUtc = ts;
            ...
            try
            {
                ...
                {
                    if (ctx.Request.Data != null && ctx.Request.Data.Length > 0)
                    {
                        byte[] bodyBytes = ctx.Request.Data;
        ...
        {
            string header = "[" + "0.0.0.0" + ":" + 0 + " CopyObject] ";
            ...
            string copySource = ctx.Http.Request.Headers.GetValueOrDefault("x-amz-copy-source", "");
            if (string.IsNullOrEmpty(copySource))
                throw new S3Exception(new Error(ErrorCode.InvalidRequest));

            copySource = Uri.UnescapeDataString(copySource).TrimStart('/');
            int slashIdx = copySource.IndexOf('/');
            if (slashIdx < 1)
                throw new S3Exception(new Error(ErrorCode.InvalidRequest));

            string srcBucketName = copySource[..slashIdx];
            string srcKey = copySource[(slashIdx + 1)..];

            // Resolve source bucket
            Classes.Bucket srcBucket = _Config.GetBucketByName(srcBucketName);
            if (srcBucket == null)
                throw new S3Exception(new Error(ErrorCode.NoSuchBucket));

            BucketClient srcClient = _Buckets.GetClient(srcBucketName);
            if (srcClient == null)
                throw new S3Exception(new Error(ErrorCode.NoSuchBucket));

            Obj srcObj = srcClient.GetObjectLatestMetadata(srcKey);
            if (srcObj == null)
                throw new S3Exception(new Error(ErrorCode.NoSuchKey));

            // Refuse to overwrite without versioning enabled
            Obj existingDest = md.BucketClient.GetObjectLatestMetadata(ctx.Request.Key);

            // Build destination object metadata
            DateTime ts = DateTime.UtcNow;
            Obj destObj = new Obj
            {
                AuthorGUID     = md.User?.GUID ?? "unknown",
                OwnerGUID      = md.User?.GUID ?? "unknown",
                GUID           = Guid.NewGuid().ToString(),
                BlobFilename   = Guid.NewGuid().ToString(),
                Version        = existingDest != null ? existingDest.Version + 1 : 1,
            ...
            // Stream source data into destination bucket
            string srcBlob = GetObjectBlobFile(srcBucket, srcObj);
            bool writeSuccess;
            using (FileStream fs = new FileStream(srcBlob, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                destObj.ContentLength = fs.Length;
                writeSuccess = md.BucketClient.AddObject(destObj, fs);
            }

            if (!writeSuccess)
            {
                _Logging.Warn(header + "failed to copy " + srcBucketName + "/" + srcKey + " → " + ctx.Request.Bucket + "/" + ctx.Request.Key);
                throw new S3Exception(new Error(ErrorCode.InternalError));
            }

            _Logging.Info(header + "copied " + srcBucketName + "/" + srcKey + " → " + ctx.Request.Bucket + "/" + ctx.Request.Key);
            return new CopyObjectResult(destObj.Etag ?? destObj.GUID, ts);
        }
        ...
        internal async Task<InitiateMultipartUploadResult> CreateMultipartUpload(S3Context ctx)
        {
            ...
            CosmoS3.Classes.Upload upload = new CosmoS3.Classes.Upload();
            ...
            {
                upload.OwnerGUID = "0.0.0.0" + ":" + 0;
                upload.AuthorGUID = "0.0.0.0" + ":" + 0;
            }

            if (ctx.Http.Request.Headers != null)
            {
                if (ctx.Http.Request.Headers.ContainsKey("content-type"))
                {
        ...
        internal async Task<CompleteMultipartUploadResult> CompleteMultipartUpload(S3Context ctx, CompleteMultipartUpload upload)
        {
            ...
            CosmoS3.Classes.Upload uploadRecord = _Config.GetUploadByGuid(ctx.Request.UploadId);
        ...
        internal async Task<ListPartsResult> ReadParts(S3Context ctx)
        {
            ...
            result.StorageClass = CosmoS3.S3Objects.StorageClassEnum.STANDARD;
        ...
        private Dictionary<string, string> ExtractMetadataFromHeaders(IReadOnlyDictionary<string, string> headers)
        {
        ...
        private List<Grant> GrantsFromHeaders(User user, IReadOnlyDictionary<string, string> headers)
        {
        ...
        /// <summary>
        /// Decodes an AWS chunked (STREAMING-AWS4-HMAC-SHA256-PAYLOAD) body into raw bytes.
        #endregion
