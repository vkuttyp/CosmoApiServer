...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        internal async Task<WebsiteConfiguration> BucketReadWebsite(S3Context ctx)
        {
            return await _BucketHandler.ReadWebsite(ctx);
        }

        internal async Task BucketWriteWebsite(S3Context ctx, WebsiteConfiguration config)
        {
            await _BucketHandler.WriteWebsite(ctx, config);
        }

        internal async Task BucketDeleteWebsite(S3Context ctx)
        {
            await _BucketHandler.DeleteWebsite(ctx);
        }
        ...
        {
            return await _ObjectHandler.Copy(ctx);
        }
