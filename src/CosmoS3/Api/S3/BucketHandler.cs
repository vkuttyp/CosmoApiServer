...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        internal async Task Delete(S3Context ctx)
        {
            ...
            // Clean up website config directory (stored separately from object data)
            string websiteDir = System.IO.Path.Combine(_Settings.Storage.DiskDirectory, md.Bucket.Name);
            if (System.IO.Directory.Exists(websiteDir))
                System.IO.Directory.Delete(websiteDir, true);
        ...
        internal async Task<ListBucketResult> Read(S3Context ctx)
        {
            ...
            Dictionary<string, CosmoS3.S3Objects.Owner> ownerCache = new Dictionary<string, CosmoS3.S3Objects.Owner>();
            ...
            foreach (Obj curr in objects)
            {
                ...
                if (fetchOwner)
                {
                    c.Owner = new CosmoS3.S3Objects.Owner();
        ...
        internal async Task<ListVersionsResult> ReadVersions(S3Context ctx)
        {
            ...
            foreach (Obj curr in objects)
            {
                if (curr.DeleteMarker)
                {
                    ...
                    d.Owner = new CosmoS3.S3Objects.Owner();
                    if (ownerCache.ContainsKey(curr.OwnerGUID))
                    {
                ...
                {
                    CosmoS3.S3Objects.ObjectVersion v = new CosmoS3.S3Objects.ObjectVersion();
                    v.ETag = null;
                    ...
                    v.Owner = new CosmoS3.S3Objects.Owner();
                    if (ownerCache.ContainsKey(curr.OwnerGUID))
                    {
        ...
        internal async Task<ListMultipartUploadsResult> ReadMultipartUploads(S3Context ctx)
        {
            ...
            List<CosmoS3.Classes.Upload> uploads = _Config.GetUploadsByBucketGuid(md.Bucket.GUID);
            if (uploads == null)
            {
                uploads = new List<CosmoS3.Classes.Upload>();
            }
            ...
            result.Uploads = new List<CosmoS3.S3Objects.Upload>();

            Dictionary<string, CosmoS3.S3Objects.Owner> ownerCache = new Dictionary<string, CosmoS3.S3Objects.Owner>();

            foreach (CosmoS3.Classes.Upload upload in uploads)
            {
                CosmoS3.S3Objects.Upload u = new CosmoS3.S3Objects.Upload();
                ...
                else
                {
                    ...
                    if (author != null)
                    {
                        CosmoS3.S3Objects.Owner initiatorOwner = new CosmoS3.S3Objects.Owner();
                ...
                else
                {
                    ...
                    if (owner != null)
                    {
                        CosmoS3.S3Objects.Owner ownerObj = new CosmoS3.S3Objects.Owner();
            ...
            {
                CosmoS3.S3Objects.Upload last = result.Uploads[result.Uploads.Count - 1];
        ...
        internal async Task<WebsiteConfiguration> ReadWebsite(S3Context ctx)
        {
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Request.RequestType.ToString() + "] ";
            RequestMetadata md = RequestValidator.ValidateAndGetMetadata(ctx, _Logging, header);
            RequestValidator.ValidateAuthorization(md, _Logging, header);
            ...
            string path = WebsiteConfigPath(md.Bucket.Name);
            if (!System.IO.File.Exists(path))
                throw new S3Exception(new Error(ErrorCode.NoSuchWebsiteConfiguration));

            string xml = await System.IO.File.ReadAllTextAsync(path);
            return SerializationHelper.DeserializeXml<WebsiteConfiguration>(xml)
                ?? new WebsiteConfiguration();
        }

        internal async Task WriteWebsite(S3Context ctx, WebsiteConfiguration config)
        {
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Request.RequestType.ToString() + "] ";
            RequestMetadata md = RequestValidator.ValidateAndGetMetadata(ctx, _Logging, header);
            RequestValidator.ValidateAuthorization(md, _Logging, header);
            ...
            await System.IO.File.WriteAllTextAsync(path, SerializationHelper.SerializeXml(config));
        }

        internal async Task DeleteWebsite(S3Context ctx)
        {
            string header = "[" + "0.0.0.0" + ":" + 0 + " " + ctx.Request.RequestType.ToString() + "] ";
            RequestMetadata md = RequestValidator.ValidateAndGetMetadata(ctx, _Logging, header);
            RequestValidator.ValidateAuthorization(md, _Logging, header);
        ...
        {
            return Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(lastId.ToString()));
        }

        private int ParseContinuationToken(string base64)
        {
            return Convert.ToInt32(System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(base64)));
        }
        ...
        private string WebsiteConfigPath(string bucketName)
            => System.IO.Path.Combine(_Settings.Storage.DiskDirectory, bucketName, "website.xml");
